...
Tested with Locust against a deliberately flaky receiver (80% success, 10% error, 5% slow, 5% disconnect). Every number below is from a committed CSV in `loadtest/`, not a one-off best-case run.
...
Three bugs caused the original bottleneck:
...
**Full pipeline reliability** (2,443 events created, tracked to completion): 2,408 delivered, 2 permanently failed after exhausting retries &mdash; **99.9% eventual delivery success among events that entered the pipeline** (2,410 of 2,443; the other 33 are Bug 4, below). Ingestion itself (measured by Locust: 2,317 requests): p50 58ms, p95 70ms, p99 75ms, 0% ingestion failures.

The gap between 2,443 created and 2,317 tracked by Locust is a fourth real bug, found by comparing Locust's own count against the actual row count in Postgres: **33 events (1.4%) were committed to the database but never entered the Redis queue at all.** Saving an event and queuing it for delivery are two separate steps, not one atomic operation &mdash; an interruption between them (here, tied to the test harness cutting off in-flight requests at the run's time limit) can silently orphan an event before it's ever queued. Not fixed; documented as a known limitation alongside the existing worker-crash-recovery gap.
