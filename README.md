...
**Full pipeline reliability** (2,443 events created): 2,408 delivered, 2 permanently failed after exhausting retries, and 33 remained orphaned/pending, never reaching a terminal state. Of the 2,410 events that entered the delivery pipeline and reached a terminal state, **99.9% were eventually delivered** (2,408 delivered, 2 failed). Ingestion itself (measured by Locust: 2,317 requests): p50 58ms, p95 70ms, p99 75ms, 0% ingestion failures.

Locust's own request count (2,317) and the actual row count in Postgres (2,443) don't match &mdash; a 126-event gap. Most of that gap is a measurement artifact of the test tool itself: Locust stops counting a request if its response arrives after the run's time-limit cutoff, even if the server already fully processed it. But inspecting the database directly (not just that count difference) surfaced a real issue inside that gap: **33 of the 2,443 events (1.4%) were stranded with no delivery attempts logged and no job left in either Redis queue** &mdash; consistent with the documented gap between committing an event to Postgres and successfully pushing it onto the Redis queue, though the evidence can't fully rule out the separate worker-crash-mid-job scenario either, since both would look identical from the outside (see Known Limitations for both). Either way, a real gap worth fixing with something like a transactional outbox pattern; not fixed here.
